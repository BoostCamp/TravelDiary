...
class Diary: Object {
    ...
    convenience init(title: String? = "", date: Date, latitude: Double? = nil, longitude: Double? = nil, text: String? = "", locatonName: String? = "", images: [UIImage]) {
        ...
        if images.count != 0 {
            self.saveImagesDocumentDirectory(images: images)
        }
    ...
    public func saveImagesDocumentDirectory(images: [UIImage]) {
        var index = 0
        for image in images {
            ...
            self.dirPathForPhotos.append(dirPathForImage)
    ...
    public func getPhotos() -> [UIImage]? {
        ...
        for index in 0..<self.dirPathForPhotos.count {
            if let photo = self.getPhoto(at: index) {
                photos += [photo]
            } else {
    ...
    public func getPhoto(at index: Int) -> UIImage? {
        let fileManager = FileManager.default
        if let path = self.dirPathForPhotos[index].dirPath {
            let imagePAth = (self.getDirectoryPath() as NSString).appendingPathComponent(path)
    ...
    public func deletePhoto() {
        let fileManager = FileManager.default
        for dirPathForPhoto in self.dirPathForPhotos {
            let paths = (self.getDirectoryPath() as NSString).appendingPathComponent(dirPathForPhoto.dirPath!)
