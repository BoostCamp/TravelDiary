...
protocol TDTableHeaderViewCellDelegate {
...
class TDTableHeaderViewCell: UITableViewCell {
    ...
    var delegate: TDTableHeaderViewCellDelegate?
    var section: Int!
    var isMoreButtonClicked: Bool = false
    var isShowCellButtonClicked: Bool = false
...
// MARK: - Actions
extension TDTableHeaderViewCell {
    
    @IBAction func showCellButtonTapped() {
        self.delegate?.showCell(header: self, section: self.section, state: self.isShowCellButtonClicked)
        if self.isShowCellButtonClicked {
